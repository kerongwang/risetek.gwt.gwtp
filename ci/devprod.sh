...
## Devops special
scp $DOCKER_PATH/prod.env $REMOTE_HOST:$WORKSPACE/.env
...
echo -e "\033[33m If prompted, you'll need to enter your sudo password\033[0m"
ssh -t $REMOTE_HOST sudo docker build $BUILD_DIR/docker/ -f $BUILD_DIR/docker/$DOCKER_FILE -t $DOCKER_IMAGE_PREFIX/$PROJECT_NAME
## clean up
...
echo -e "\033[33m If prompted, you'll need to enter your sudo password\033[0m"
ssh -t $REMOTE_HOST "cd $WORKSPACE && sudo docker-compose -f $WORKSPACE/docker-compose.yml down || true && sudo docker-compose -f $WORKSPACE/docker-compose.yml up -d"
